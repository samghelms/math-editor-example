  ...
  return _ref3 = {
    ...
    if (key === Keyboard.keys.LEFT) {
      if (shiftKey) {
        ...
        leaf.enter([range.index - 1, range.length + 1, _quill2.default.sources.USER]);
      }
    } else {
      if (shiftKey) {
        this.quill.setSelection(range.index, range.length + 1, _quill2.default.sources.USER);
      } else {
        leaf.enter([range.index + 1, range.length, _quill2.default.sources.USER]);
      }
    }
    return false;
  }), _ref3;
...
var Embed = function (_Parchment$Embed) {
  ...
  _createClass(Embed, [{
    ...
    key: 'enter',
    value: function enter(quill, args) {
      quill.setSelection.apply(quill, _toConsumableArray(args));
    }
  }, {
    key: 'update',
    value: function update(mutations, context) {
      ...
      handlers: {
        formula: function formula() {
          ...
          if (range != null) {
            ...
            this.quill.insertEmbed(index, "formula", value, _emitter2.default.sources.USER);
            this.quill.insertText(index + 1, value, _emitter2.default.sources.USER);

            var _quill$getLeaf = this.quill.getLeaf(index + 1),
...
var TOOLBAR_CONFIG = [[{ header: ['1', '2', '3', false] }], ['bold', 'italic', 'underline', 'link'], [{ list: 'ordered' }, { list: 'bullet' }], ['clean'], ['formula']];
...
var Formula = function (_Embed) {
  ...
  _createClass(Formula, null, [{
    key: "create",
    value: function create(value) {
      var node = _get(Formula.__proto__ || Object.getPrototypeOf(Formula), "create", this).call(this, value);
  ...
  function Formula(value) {
    ...
    var _this = _possibleConstructorReturn(this, (Formula.__proto__ || Object.getPrototypeOf(Formula)).call(this, value));

    var mathNode = getMathNode(_this.domNode);
    _this.renderMath(value.getAttribute("data-value"), mathNode);
    mathNode.addEventListener('click', function () {
      return _this.edit([-1]);
    });
    return _this;
  }
  ...
  _createClass(Formula, [{
    ...
    value: function exit() {
      ...
      /* These indicing assumptions are safe since we created all of the elements
      in other parts of this class*/
      var parent = this.domNode.children[0];
      var editContainers = parent.getElementsByClassName("editContainer");
      var editContainer = editContainers[0];
      var mathNode = getMathNode(editContainer);
      parent.replaceChild(mathNode, editContainer);
    }
  }, {
    key: "edit",
    value: function edit(args) {
      var mathNodes = this.domNode.getElementsByClassName("mathNode");
      if (mathNodes.length !== 1) {
        throw new Error();
      }
      var mathNode = mathNodes[0];
      var mathNodeParent = mathNode.parentNode;
      var editContainer = this.editContainer();
      mathNodeParent.removeChild(mathNode);
      editContainer.appendChild(mathNode);
      mathNodeParent.appendChild(editContainer);
      var editArea = this.getEditor();
      var editAreaContainer = document.createElement("div");
      editAreaContainer.appendChild(editArea);
      var val = this.contentNode.getAttribute("data-value");
      editArea.value = val;
    ...
    value: function getEditor() {
      ...
      editArea.addEventListener('input', function (el) {
        return _this2.inputHandler(el);
      });
      editArea.addEventListener("keydown", function (e) {
        return keyHandler(e, _this2);
      });
    ...
    key: "renderMath",
    value: function renderMath(value, mathNode) {
      var processedValue = this.preprocessor(value);
      try {
    ...
    value: function inputHandler(e) {
      var newVal = e.srcElement.value;
      var mathNode = getMathNode(this.domNode);

      this.renderMath(newVal, mathNode);
      this.contentNode.setAttribute('data-value', newVal);
      this.domNode.setAttribute('data-value', newVal);
    }
  }], [{
    key: "value",
    value: function value(domNode) {
      var val = domNode.getAttribute('data-value');
      return val;
    }
  }]);
...
var keyHandler = function keyHandler(e, ctx) {
  var selectionLength = e.srcElement.value.length;
  var cursorLocation = e.srcElement.selectionEnd;
  var key = e.key;
  var curIndex = window.quill.getIndex(ctx);
...
var getMathNode = function getMathNode(domNode) {
  var mathNodes = domNode.getElementsByClassName("mathNode");
  if (mathNodes.length !== 1) {
    var err = new Error("should not have multiple math classes in the same embed");
    throw err;
  } else {
    return mathNodes[0];
  }
